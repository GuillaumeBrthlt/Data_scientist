...
def five_size
  handles = inputs
  handles.filter! {|element| element.length == 6 }
  handles.count
end

def start_capital
  handles = inputs
  handles.filter! {|element| element[1].match( /[[:upper:]]/ )}
  handles.count
end
...
def last
  for i in 3..32
    handles = inputs
    handles.filter! {|element| element.length == (i + 1) }
    nombre = handles.count
    if nombre != 0
      puts "il y a #{nombre} handle(s) comportant #{i} caractères."
    end
  end
end
